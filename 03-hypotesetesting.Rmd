...
ggplot(kurve, aes(x, tetthet)) +
  ...
  annotate("segment", x = obs, xend = obs, y = 0, yend = dchisq(obs, df = 2),
           colour = "firebrick", linewidth = 0.9) +
  annotate("text", x = 9.4, y = 0.055, label = "Forkastnings-\nområde (5 %)", size = 3) +
  annotate("text", x = 3.0, y = 0.16, label = "kritisk verdi\n5.99", size = 3) +
  annotate("segment", x = 3.4, xend = krit, y = 0.13, yend = 0.012,
           arrow = arrow(length = unit(0.15, "cm"))) +
  annotate("text", x = 11.7, y = 0.10, label = "observert\nverdi 8.18", colour = "firebrick", size = 3) +
  annotate("segment", x = 11.2, xend = obs, y = 0.078, yend = 0.008,
           colour = "firebrick", arrow = arrow(length = unit(0.15, "cm"))) +
...
Testobservatoren (`X-squared` $\approx 1815$) er enormt mye større enn den kritiske verdien (43.8), og **p-verdien** er forsvinnende liten, så vi forkaster $H_0$: holdning til innvandring henger klart sammen med politisk ståsted. (Sjekk gjerne med `chisq.test(immigration)$expected` at ingen forventede celletall er under 5, ellers er tilnærmingen upålitelig.)
