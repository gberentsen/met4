...
a. Test på 5 %-nivå om populasjonsgjennomsnittet til `X1` er lik 100, mot det tosidige alternativet. Bruk `t.test()`. Hva blir konklusjonen?

b. Test på 5 %-nivå om `X1` og `X2` har samme populasjonsgjennomsnitt. Se på **p-verdien** i utskriften og konkluder.
...
Testobservatorene må da sammenlignes med relevant kvantil i normalfordelingen.
...
For to-utvalgs varianstesten (F-testen) bruker vi F-fordelingen. Setter vi den største variansen i telleren, $F = s_1^2/s_2^2$, trenger vi bare øvre hale, uansett om testen er ensidig eller tosidig. Har vi $30$ observasjoner bak $s_1^2$ og $25$ bak $s_2^2$, er kritisk verdi for en tosidig test ($H_A: \sigma_1^2 \neq \sigma_2^2$):
```{r, eval = TRUE}
...
Hvis vi skal teste for goodness-of-fit for en fordeling kan vi raskt regne ut testobservatoren direkte:
