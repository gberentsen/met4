...
**Signifikansnivået** $\alpha$ er sannsynligheten vi på forhånd er villige til å godta for å gjøre en Type I-feil: $\alpha = P(\text{forkaste } H_0 \mid H_0 \text{ er sann})$. Vi velger $\alpha$ før vi ser dataene (typisk 5 %), og det er $\alpha$ som bestemmer hvor grensen for forkastningsområdet går. Et 5 %-nivå betyr at vi legger forkastningsområdet der de 5 % mest ekstreme verdiene under $H_0$ ligger.
...
Vi går gjennom de tre i tur og orden.
...
**Eksempel.** En kaffebar hevder at den gjennomsnittlige ventetiden i kø er 5 minutter. Vi mistenker at den egentlig er lengre, og følger de fem stegene:

1. **Spørsmål:** Er den forventede ventetiden lengre enn 5 minutter?
2. **Hypoteser:** $H_0: \mu = 5$ mot $H_A: \mu > 5$ (ensidig, siden vi bare er interessert i om den er *lengre*).
3. **Data:** Vi måler ventetiden (i minutter) til 30 tilfeldig valgte kunder. Vi simulerer dataene selv, så du kan kjøre koden og få nøyaktig de samme tallene:

```{r}
set.seed(1)
ventetid <- rnorm(30, mean = 5.4, sd = 1.2)
```

4. **Testobservator:** Vi kjenner ikke det sanne standardavviket, så vi bruker $T = (\overline{X} - \mu_0)/(s/\sqrt{n})$. Vi bruker R som kalkulator:

```{r}
gj.snitt <- mean(ventetid)     # utvalgsgjennomsnittet
s        <- sd(ventetid)       # utvalgsstandardavviket
n        <- length(ventetid)   # antall observasjoner
mu0      <- 5                  # verdien fra nullhypotesen

T_obs <- (gj.snitt - mu0)/(s/sqrt(n))
T_obs
```

5. **Kritisk verdi og konklusjon:** Testobservatoren er t-fordelt med $n - 1 = 29$ frihetsgrader. For en ensidig test på 5 %-nivå er den kritiske verdien 95-prosentilen $t_{0.95,\,29}$, som vi henter fra t-fordelingen (se avsnittet om kritiske verdier i \@ref(relevante-r-testing)):

```{r}
qt(0.95, df = n - 1)
```

Testobservatoren (2.46) er større enn den kritiske verdien (1.70), så den lander i forkastningsområdet. Vi forkaster $H_0$ og konkluderer med at den forventede ventetiden er lengre enn 5 minutter.

**I R** kan vi gjøre hele testen på én gang med `t.test()`:

```{r}
t.test(ventetid, mu = 5, alternative = "greater")
```

I utskriften er det særlig **p-verdien** du skal se på. Her er den godt under 0.05, så vi forkaster $H_0$, akkurat som med den kritiske verdien. Utskriften gir også selve testobservatoren (`t`), antall frihetsgrader (`df`) og et konfidensintervall for den forventede ventetiden.

### Inferens om en andel

Ofte er størrelsen vi er ute etter, en **andel** $p$, for eksempel andelen velgere som støtter et parti, eller andelen defekte enheter i en produksjon. Vi tester da $H_0: p = p_0$. Fra samplingfordelingen til en andel (modul 2) vet vi at den observerte andelen $\widehat{p}$ er tilnærmet normalfordelt for store utvalg. Under nullhypotesen er $p = p_0$, så testobservatoren blir
...
**Eksempel.** Vi vender tilbake til partiet fra forrige delkapittel: har det oppslutning over sperregrensen på 4 %?

1. **Spørsmål:** Er den sanne oppslutningen $p$ større enn 0.04?
2. **Hypoteser:** $H_0: p = 0.04$ mot $H_A: p > 0.04$ (ensidig).
3. **Data:** Vi spør 1000 tilfeldig valgte velgere, og 55 av dem svarer at de vil stemme partiet. Den observerte andelen er $\widehat{p} = 55/1000 = 0.055$.
4. **Testobservator:** Under $H_0$ er $p_0 = 0.04$, så
...
5. **Kritisk verdi og konklusjon:** Testobservatoren er tilnærmet standardnormalfordelt. For en ensidig test på 5 %-nivå er den kritiske verdien $z_{0.95} = 1.645$ (`qnorm(0.95)`, se \@ref(relevante-r-testing)). Siden $2.42 > 1.645$, forkaster vi $H_0$: resultatet tyder på oppslutning over sperregrensen.

**I R** kan vi bruke `prop.test()`, som tar antall «suksesser», antall forsøk, verdien under $H_0$ og retningen på testen:

```{r}
prop.test(55, 1000, p = 0.04, alternative = "greater", correct = FALSE)
```

Igjen er det **p-verdien** du skal se på. Den er 0.008, altså under 0.05, så vi forkaster $H_0$. (`prop.test()` rapporterer en `X-squared`-verdi som rett og slett er $Z^2$. Vi bruker `correct = FALSE` for å få nøyaktig samme test som den vi regnet ut for hånd.)

### Inferens om et standardavvik

Noen ganger er det *spredningen* i seg selv vi vil teste, for eksempel om en maskin produserer enheter med et standardavvik som er større enn en tillatt grense. Vi tester da $H_0: \sigma = \sigma_0$ (likeverdig med $\sigma^2 = \sigma_0^2$), og bruker testobservatoren
...
**Eksempel.** En maskin skal fylle flasker slik at standardavviket i fyllmengde er høyst 2 ml. Vi vil teste om spredningen egentlig er større enn dette.

1. **Spørsmål:** Er standardavviket $\sigma$ større enn 2 ml?
2. **Hypoteser:** $H_0: \sigma = 2$ mot $H_A: \sigma > 2$ (ensidig).
3. **Data:** Vi måler fyllmengden (i ml) til 20 tilfeldige flasker (simulert):

```{r}
set.seed(2)
fyllmengde <- rnorm(20, mean = 500, sd = 2.6)
```

4. **Testobservator:** Vi bruker $\chi^2 = (n-1)s^2/\sigma_0^2$ med $\sigma_0 = 2$:

```{r}
s      <- sd(fyllmengde)       # utvalgsstandardavviket
n      <- length(fyllmengde)   # antall observasjoner
sigma0 <- 2                    # standardavviket under H_0

chikvadrat <- (n - 1)*s^2/sigma0^2
chikvadrat
```

5. **Kritisk verdi og konklusjon:** Testobservatoren er kji-kvadratfordelt med $n - 1 = 19$ frihetsgrader. Siden alternativet er $\sigma > 2$, ligger forkastningsområdet i *øvre* hale, og den kritiske verdien er 95-prosentilen i kji-kvadratfordelingen (`qchisq(0.95, df = 19)`, se \@ref(relevante-r-testing)):

```{r}
qchisq(0.95, df = n - 1)
```

Testobservatoren (36.0) er større enn den kritiske verdien (30.1), så vi forkaster $H_0$: spredningen ser ut til å være større enn kravet på 2 ml.

**I R** finnes det ingen ferdig funksjon for denne testen slik som `t.test()` og `prop.test()`, så vi gjør den «for hånd» i R slik vi nettopp gjorde. Vil du heller ha p-verdien direkte enn å sammenligne med en kritisk verdi, finner du den som arealet i øvre hale med `pchisq()`:

```{r}
1 - pchisq(chikvadrat, df = n - 1)
```

p-verdien er under 0.05, så konklusjonen er den samme: vi forkaster $H_0$.
