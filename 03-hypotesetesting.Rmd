...
La oss gjennomføre en t-test for et gjennomsnitt i R. En kaffebar hevder at den gjennomsnittlige ventetiden i kø er 5 minutter. Vi mistenker at den egentlig er lengre, og måler ventetiden (i minutter) til 30 tilfeldig valgte kunder. Vi simulerer dataene selv, så du kan kjøre koden og få de samme tallene:

```{r}
set.seed(1)
ventetid <- rnorm(30, mean = 5.4, sd = 1.2)   # 30 simulerte ventetider
```
...
altså en ensidig test, siden vi bare er interessert i om ventetiden er *lengre* enn påstått. Vi regner først ut testobservatoren $T$ for hånd:
...
Den kritiske verdien for en ensidig test på 5 %-nivå henter vi fra t-fordelingen med $n-1 = 29$ frihetsgrader:

```{r}
qt(0.95, df = n - 1)
```

Siden testobservatoren er større enn den kritiske verdien, forkaster vi $H_0$: dataene tyder på at den forventede ventetiden er lengre enn 5 minutter. Vi kunne fått nøyaktig samme konklusjon ved å bruke den innebygde `t.test()`-funksjonen direkte, som i tillegg gir oss p-verdien:

```{r}
t.test(ventetid, mu = 5, alternative = "greater")
```

p-verdien er langt mindre enn 0.05, så vi forkaster $H_0$, akkurat som over.
