...
**Eksempel.** NHHs karrieresenter oppgir at gjennomsnittlig startlønn for nyutdannede masterstudenter er 620 000 kr. En studentforening mistenker at den egentlig er høyere, og gjennomfører en spørreundersøkelse. Vi følger de fem stegene:

1. **Spørsmål:** Er den forventede startlønnen høyere enn 620 000 kr?
2. **Hypoteser og signifikansnivå:** $H_0: \mu = 620\,000$ mot $H_A: \mu > 620\,000$ (ensidig). Vi bestemmer *på forhånd*, før vi ser på dataene, at vi vil bruke et signifikansnivå på $\alpha = 5\,\%$.
3. **Data:** Vi spør 40 tilfeldig valgte nyutdannede om startlønnen deres. Vi simulerer svarene, så du kan kjøre koden og få de samme tallene:

```{r}
set.seed(2)
lonn <- rnorm(40, mean = 636000, sd = 52000)
```
...
```{r}
gj.snitt <- mean(lonn)     # utvalgsgjennomsnittet
s        <- sd(lonn)       # utvalgsstandardavviket
n        <- length(lonn)   # antall observasjoner
mu0      <- 620000         # verdien fra nullhypotesen
...
Testobservatoren (2.31) er større enn den kritiske verdien (1.69), så den lander i forkastningsområdet. Vi forkaster $H_0$ og konkluderer med at den forventede startlønnen er høyere enn 620 000 kr.

Over gjorde vi testen manuelt, steg for steg. **I R** kan vi i stedet la `t.test()` gjøre hele jobben direkte fra dataene:

```{r}
t.test(lonn, mu = 620000, alternative = "greater")
```

I utskriften er det særlig **p-verdien** du skal se på: er den mindre enn signifikansnivået vi valgte på forhånd ($\alpha = 0.05$), forkaster vi $H_0$. Her er p-verdien 0.013, altså under 0.05, så vi forkaster, akkurat som med den kritiske verdien. Utskriften gir også selve testobservatoren (`t`), antall frihetsgrader (`df`) og et konfidensintervall for den forventede startlønnen.
...
2. **Hypoteser og signifikansnivå:** $H_0: p = 0.05$ mot $H_A: p > 0.05$ (ensidig). Vi velger på forhånd et signifikansnivå på $\alpha = 5\,\%$.
...
2. **Hypoteser og signifikansnivå:** $H_0: \sigma = 4$ mot $H_A: \sigma > 4$ (ensidig). Vi velger på forhånd et signifikansnivå på $\alpha = 5\,\%$.
