...
For tester på andeler bruker vi `prop.test()`, som tar antall "suksesser" og antall forsøk. I testdatasettet er `A1` og `A2` 0/1-kolonner, så antall suksesser er `sum(testdata$A1)` og antall forsøk `length(testdata$A1)`.

#### Ett-utvalgs test for en andel {-}

Tester om en andel er lik en bestemt verdi, her $H_0: p = 0.5$:
```{r}
...
Sammenligner to andeler, $H_0: p_1 = p_2$:
```{r}
prop.test(c(sum(testdata$A1), sum(testdata$A2)),
          c(length(testdata$A1), length(testdata$A2)), correct = FALSE)
```

Vi bruker `correct = FALSE` for å få nøyaktig samme test som Z-observatoren fra teorien (uten kontinuitetskorreksjon), og gjør testene ensidige med `alternative = "greater"` eller `"less"`. Utskriften rapporterer `X-squared`, som rett og slett er kvadratet av Z-observatoren.
