...
aliases:
  - enpop.html
  - generelt-om-hypotesetesting.html
  - hypotesetesting.html
  - inferens-om-to-populasjoner.html
  - kjikvadrattester.html
  - oppgaver.html
  - relevante-r-testing.html
---
...
## Generelt om hypotesetesting {#sec-generelt-om-hypotesetesting}

### Videoforelesninger {.unnumbered}
...
**Forkastningsområdet** er de verdiene av testobservatoren som skal få oss til å forkaste. Det velges direkte ut fra fordelingen og signifikansnivået: vi legger det i den halen (eller de halene) av fordelingen som svarer til $H_A$, og gjør det akkurat stort nok til at sannsynligheten for å havne der er $\alpha$ når $H_0$ er sann. Grensen kalles den **kritiske verdien**. I Venstre-eksempelet er $H_A$ ensidig ($p > p_0$), så hele forkastningsområdet ligger i høyre hale, og den kritiske verdien er 95-prosentilen i standardnormalfordelingen, $z_{0.95} = 1.645$. Vi forkaster dersom $Z > 1.645$, og siden den observerte verdien $Z = 2.42$ ligger godt innenfor, forkaster vi (@fig-forkastning).
...
5. **Kritisk verdi og konklusjon:** Testobservatoren er t-fordelt med $n - 1 = 39$ frihetsgrader. For en ensidig test på 5 %-nivå er den kritiske verdien 95-prosentilen $t_{0.95,\,39}$ (se avsnittet om kritiske verdier i @sec-relevante-r-testing):
...
5. **Kritisk verdi og konklusjon:** Testobservatoren er tilnærmet standardnormalfordelt. For en ensidig test på 1 %-nivå er den kritiske verdien $z_{0.99}$ (se @sec-relevante-r-testing):
...
5. **Kritisk verdi og konklusjon:** Testobservatoren er kji-kvadratfordelt med $n - 1 = 29$ frihetsgrader. Siden alternativet er $\sigma > 4$, ligger forkastningsområdet i *øvre* hale, og den kritiske verdien er 95-prosentilen (se @sec-relevante-r-testing):
...
5. **Kritisk verdi og konklusjon:** Testobservatoren er t-fordelt med $n_1 + n_2 - 2 = 98$ frihetsgrader. Kritisk verdi for en ensidig test på 10 %-nivå er 90-prosentilen $t_{0.90,\,98}$ (se @sec-relevante-r-testing):
...
5. **Kritisk verdi og konklusjon:** Differansene gir $n - 1 = 11$ frihetsgrader. Kritisk verdi for en ensidig test på 5 %-nivå er $t_{0.95,\,11}$ (se @sec-relevante-r-testing):
...
Et nyttig triks er å alltid sette den *største* utvalgsvariansen i telleren. Da blir $F > 1$, og vi trenger bare å sammenligne med den kritiske verdien i *høyre* hale av F-fordelingen, uansett om testen er ensidig eller tosidig (se @sec-relevante-r-testing).
...
5. **Kritisk verdi og konklusjon:** $F$ er F-fordelt med $(n_1 - 1, n_2 - 1) = (29, 29)$ frihetsgrader. Kritisk verdi for en ensidig test på 5 %-nivå er 95-prosentilen i F-fordelingen (se @sec-relevante-r-testing):
...
Her ligger de to fondene i hver sin vektor. Kommer dataene i stedet som ett datasett med en kategorisk variabel som angir gruppen, tar `var.test()` også formelsyntaks, `var.test(verdi ~ gruppe, data = df)`, på samme måte som `t.test()`. Og vil du bare ha ett av tallene i utskriften, henter du det ut med `$`, for eksempel `var.test(...)$p.value`. Begge deler er beskrevet i @sec-relevante-r-testing.
...
5. **Kritisk verdi og konklusjon:** For en ensidig test på 10 %-nivå er den kritiske verdien $z_{0.90}$ (se @sec-relevante-r-testing):
...
Her øver vi på å kjøre hver av testene fra kapittelet i R. Oppgavene er korte: les inn de oppgitte tallene, kjør riktig test, og konkluder ut fra **p-verdien**. (I praksis leser du gjerne dataene fra en fil, se @sec-relevante-r-testing; her skriver vi dem inn direkte for enkelhets skyld.) Alle testene bruker 5 %-signifikansnivå.
...
```{r}
ventetid <- c(3.4, 4.1, 2.8, 3.9, 4.5, 3.2, 5.0, 3.7, 4.3, 3.6)
```

Test om den forventede ventetiden er *lengre* enn 3 minutter.
...
p-verdien er 0.001, godt under 0.05, så vi forkaster $H_0$: ventetiden ser ut til å være lengre enn 3 minutter.
...
metode_b <- c(80, 84, 79, 88, 76, 82, 90)
```

Test om de to metodene gir ulik forventet poengsum.
...
p-verdien er 0.004, under 0.05, så vi forkaster $H_0$: metodene gir ulik poengsum (metode B ser ut til å være best).
...
etter_kurs <- c(60, 66, 59, 71, 68, 65, 67, 63)
```

Siden det er de *samme* personene før og etter, er dataene paret. Test om kurset økte den forventede produktiviteten.
...
p-verdien er 0.0004, så vi forkaster $H_0$: kurset ser ut til å ha økt produktiviteten.
...
p-verdien er 0.0008, så vi forkaster $H_0$: klikkraten (nå $\widehat{p} = 46/200 = 0.23$) ser ut til å ha økt.
...
p-verdien er 0.0015, så vi forkaster $H_0$: versjon A ser ut til å ha høyere konverteringsrate.
...
```{r}
vekt <- c(9.8, 10.4, 9.6, 10.9, 8.7, 11.2, 9.1, 10.6, 9.9, 10.1, 8.9, 11.0)
```

Test (tosidig) om standardavviket er forskjellig fra 1 gram. Det finnes ingen ferdig funksjon for denne testen, så du må regne ut testobservatoren selv (se @sec-relevante-r-testing).
...
# Kritiske verdier for en tosidig test:
c(qchisq(0.025, df = n - 1), qchisq(0.975, df = n - 1))
```

Testobservatoren (7.70) ligger *innenfor* de kritiske verdiene (3.82 og 21.92), så vi kan ikke forkaste $H_0$: standardavviket er forenlig med 1 gram.
...
maskin2 <- c(50.1, 49.9, 50.2, 49.8, 50.3, 49.7, 50.0, 50.0)
```

Test om de to maskinene har ulik varians.
...
p-verdien er 0.005, under 0.05, så vi forkaster $H_0$: maskinene har ulik varians (maskin 1 er tydelig mindre presis).
...
```{r}
kast <- c(8, 13, 9, 11, 7, 12)   # antall 1-ere, 2-ere, ..., 6-ere
```

Test om terningen er rettferdig (alle øyne like sannsynlige).
...
p-verdien er 0.73, langt over 0.05, så vi kan ikke forkaste $H_0$: resultatene er forenlige med en rettferdig terning.
...
p-verdien er 0.0001, under 0.05, så vi forkaster $H_0$: det er en sammenheng mellom kjønn og kjøp.
...
## Relevante R-kommandoer {#sec-relevante-r-testing}
...
Vi følger samme rekkefølge som i teorien: først tester for et gjennomsnitt og for differansen mellom to gjennomsnitt (t-tester), så tester for andeler, så varianstester, og til slutt kji-kvadrattester. De fleste testene kjører vi direkte med en ferdig funksjon (`t.test()`, `var.test()`, `chisq.test()` og så videre), som gir både testobservator og p-verdi. Trenger du i stedet en *kritisk verdi* (for eksempel på eksamen), finner du den med **kvantilfunksjonene** `qt()`, `qnorm()`, `qchisq()` og `qf()`. Under hver testtype viser vi begge deler. (Kvantil- og sannsynlighetsfunksjonene ble introdusert i modul 2, se @sec-relevant-r-grunnleggende.)

### t-tester

Du må kunne gjøre de ulike $t$-testene ved hjelp av `t.test()`-funksjonen. Vi bruker eksempeldatasettet [testdata.xls](datasett/testdata.xls) som illustrasjon, som inneholder de to kolonnene `X1` og `X2`. I bunn og grunn handler det om å hente ut de relevante kolonnene som vektorer og sette argumentene i `t.test()` riktig.
```{r, echo = FALSE}
testdata <- readxl::read_excel("datasett/testdata.xls")
```

#### Ett-utvalgs t-test {.unnumbered}
...
I alle tre tilfellene kan du gjøre testen ensidig ved å sette `alternative = "greater"` eller `alternative = "less"`.

#### Ensidige to-utvalgs tester: Hvilken vei tester vi? {.unnumbered}
...
#### Bruke formel-syntaks med `~` {.unnumbered}
...
#### Kritiske verdier i t-fordelingen {.unnumbered}
...
qt(0.05, df = 40 - 1)
```
og du forkaster dersom testobservatoren $T$ er *mindre* enn dette tallet. Mot $H_A: \mu > \mu_0$ bruker du øvre kvantil:
```{r, eval = TRUE}
qt(1 - 0.05, df = 40 - 1)
```
og forkaster dersom $T$ er *større*. For en tosidig test ($H_A: \mu \neq \mu_0$) trenger du begge grensene, og forkaster dersom $T$ faller utenfor:
```{r, eval = TRUE}
L <- qt(0.05/2, df = 40 - 1)
...
#### Ett-utvalgs test for en andel {.unnumbered}
...
- **`p-value`** er tallet du konkluderer med: er den under signifikansnivået, forkaster du $H_0$.
- **`X-squared`** er testobservatoren. `prop.test()` rapporterer den som en kji-kvadratverdi i stedet for en Z-verdi, men det er *nøyaktig* samme test: `X-squared` er rett og slett $Z^2$ (så $Z = \sqrt{\texttt{X-squared}}$).
- **`sample estimates`** nederst er den observerte andelen $\widehat{p}$, og linjen over er et konfidensintervall for den sanne andelen.
...
#### To-utvalgs test for to andeler {.unnumbered}
...
Nå viser `sample estimates` de to andelene (`prop 1` og `prop 2`), og resten leses på samme måte som over.

#### Regne ut Z-observatoren for hånd {.unnumbered}
...
```
Testobservatoren sammenlignes så med relevant kvantil i normalfordelingen.

#### Kritiske verdier i standardnormalfordelingen {.unnumbered}
...
```{r, eval = TRUE}
qnorm(0.05)         # ensidig, H_A: p < p0
qnorm(1 - 0.05)     # ensidig, H_A: p > p0
...
#### Ett-utvalgs varianstest {.unnumbered}
...
Vi må så sammenligne verdien av testobservatoren `chikvadrat` med de kritiske verdiene `L` og `U`. Legg merke til at vi har brukt en funksjon i R (`qchisq()`) for å finne de aktuelle kvantilene i kjikvadratfordelingen.

#### Kritiske verdier i $\chi^2$-fordelingen {.unnumbered}
...
```{r, eval = TRUE}
qchisq(0.05, df = 30)   # nedre hale, 30 frihetsgrader
```
Mot $H_A: \sigma^2 > \sigma_0^2$ forkaster vi i *øvre* hale:
```{r, eval = TRUE}
qchisq(1 - 0.05, df = 30)
```
...
#### To-utvalgs varianstest {.unnumbered}
...
`var.test()` tar også **formelsyntaks**, på nøyaktig samme måte som `t.test()`. Har du ett datasett `df` der den numeriske variabelen `verdi` skal sammenlignes mellom nivåene i den kategoriske variabelen `gruppe`, slipper du å plukke ut to vektorer først:

```{r, eval = FALSE}
var.test(verdi ~ gruppe, data = df)
```

Alt som står om nivårekkefølge i avsnittet om formelsyntaks for `t.test()` over gjelder også her: R danner forholdet mellom variansene i den rekkefølgen nivåene har. Dette er ofte den raskeste veien når du skal sjekke om du kan bruke `var.equal = TRUE` i en påfølgende `t.test()` på det samme datasettet, siden begge testene da kan skrives på nøyaktig samme form.

#### Kritiske verdier i F-fordelingen {.unnumbered}
...
```
og for en ensidig test ($H_A: \sigma_1^2 > \sigma_2^2$):
```{r, eval = TRUE}
qf(1 - 0.05, df1 = 30 - 1, df2 = 25 - 1)
```
Husk: du kan selv velge hva som er $s_1^2$ og $s_2^2$, og med strategien over lar du alltid $s_1^2$ være den største av de to variansene.
...
Denne matrisen sender vi rett inn i `chisq.test()`, akkurat som krysstabellen over:
...
qchisq(1 - 0.05, df = 2)
```
og du forkaster dersom testobservatoren er større enn dette tallet. For en uavhengighetstest er antall frihetsgrader $(r-1)(s-1)$.

### Hente ut enkelttall fra en test {#sec-hente-ut-testverdier}

`t.test()`, `var.test()`, `prop.test()` og `chisq.test()` skriver ut en hel rapport når du kjører dem. Noen ganger vil du bare ha ett av tallene, for eksempel fordi du skal regne videre på det, sammenligne mange tester, eller svare på et spørsmål som bare ber om p-verdien. Da lagrer du testen i en variabel og henter ut den enkelte komponenten med `$`:

```{r}
test <- t.test(testdata$X1, mu = 100)

test$p.value      # p-verdien
test$statistic    # testobservatoren
test$conf.int     # konfidensintervallet
test$estimate     # utvalgsgjennomsnittet
```

Trenger du bare ett tall, kan du hekte `$` rett på funksjonskallet:

```{r}
t.test(testdata$X1, mu = 100)$p.value
```

Navnene er de samme for de andre testfunksjonene, med to unntak verdt å merke seg: `chisq.test()` har verken `estimate` eller `conf.int`, men til gjengjeld `expected` (de forventede frekvensene under $H_0$), og `estimate` i `var.test()` er *forholdet* mellom de to utvalgsvariansene, ikke et gjennomsnitt.

Vil du se hvilke komponenter en bestemt test har, gir `names()` deg hele lista:

```{r}
names(var.test(testdata$X1, testdata$X2))
```
