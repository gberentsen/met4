...
- `01-`…`06-*.qmd` – teorimodulene, i rekkefølge. Mal per deltema: `### Videoforelesninger {.unnumbered}` (skal *ikke* ha kapittelnummer) → teoritekst delt i vanlige `###`-underseksjoner (bruk *ikke* en egen «Teori»-overskrift, slik at første underseksjon blir f.eks. 2.2.1) → `### Kontrollspørsmål` helt til slutt i deltemaet. Oppgavene samles i en egen `## Oppgaver`-seksjon per modul, delt i tre `###`-kategorier: **Teorioppgaver** (uten R, relativt enkle), **Nøtter** (vanskeligere) og **R-oppgaver**. Modul 3 er ryddet til denne malen; modul 4–6 har foreløpig eldre struktur (`### Kommentarer` med lærebok-henvisninger) og skal ryddes på samme måte. **Modul 2 er flatet ut** (besluttet): deltema-nivået «Deskriptiv statistikk»/«Utvalg og estimering» er fjernet, hvert tema er en `##`-seksjon med sin egen video rett under overskriften, introen til begge halvdelene står samlet øverst i kapittelet, og det er én `## Kontrollspørsmål` til slutt (før `## Oppgaver`). Ikke gjeninnfør deltema-nivået her.
