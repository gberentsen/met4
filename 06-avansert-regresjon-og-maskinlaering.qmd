---
aliases:
  - avansert-regresjon-og-maskinlring.html
  - identifikasjon-og-kausalitet.html
  - introduksjon-til-maskinlring-med-knn.html
  - logistisk-regresjon.html
  - oppgaver-3.html
  - paneldata.html
  - relevante-r-avansert-regresjon.html
---
...
```{r fig-churn, message = FALSE, echo = FALSE, fig.height = 5, fig.cap = "Røde prikker er kunder som har sagt opp abbonnementet sitt, svarte prikker er kunder som ikke har gjort det. Finn den optimale avveiningen mellom systematikk og tilfeldig variasjon."}
library(ggplot2)
library(dplyr)
...
hvor vi nå har lagt til to nye ledd, $\nu_t$ og $\alpha_i$, i modellen:
...
Gitt et estimat av $\beta_1$ kan vi få estimater av $\alpha_1, \dots, \alpha_N$ ved å  og ta forventning av den tidsgjennomsnittlige modellen @eq-timemean (da forsvinner $\overline{\epsilon}_{it}$ leddet), erstatte $\beta_1$ med estimatet $\hat{\beta}_1$ og løse ligningen m.h.p $\alpha_i$.  
...
<!-- Vi husker at det også kunne være en felles tidskomponent $\nu_t$ i en slik modell og at denne kunne betraktes som en kategorisk variabel siden det typisk ikke er så mange ($T$) av disse. Vi kan derfor bare legge til `year`  som en ekstra (kategorisk) forklaringsvariabel forutsatt at den er koded som en `factor`:-->
...
*$\alpha_i\sim N(0, \sigma_{\alpha}^2)$ dersom vi bruker parameteriseringen @eq-withb med $\beta_0$ inkludert.  
* $\alpha_i\sim N(\beta_0, \sigma_{\alpha}^2)$ dersom vi bruker parameteriseringen @eq-withoutb uten $\beta_0$.  
...
summary(reg.re)
```
I motsetningen til modellen vi estimerte med faste effekter er nå parameteriseringen gjort med $\beta_0$ leddet inkludert (modell @eq-withb) og vi ser at dette blir estimert til å være $-1.35$. Videre estimeres effekten av å jobbe mer til $\hat{\beta}_1 = 0.54$. Litt lenger oppe i utskriften ser vi at  $\hat{\sigma}^2_{\alpha}= 0.0242$ ("individual"), mens $\hat{\sigma}^2_{\epsilon}= 0.0134$ ("idiosyncratic").   
...
### Eksogenitet som identifikasjonsbetingelse: Når kan vi gi en statistisk sammenheng en kausal tolkning? {#sec-ekso}
...
$$y_{i}=\beta_0+\beta_1x_{i}+\epsilon_{i}$$
Med denne likningen deler vi opp $y_i$ i tre deler: Konstantleddet $\beta_0$, det $x$ forklarer ($\beta_1x_i$) og det $x$ ikke forklarer (restleddet $\epsilon_i$). 

Én sentral betingelse for OLS var **eksogenitet**, nemlig at forventningsverdien til restleddet $\epsilon_i$ gitt forklaringsvariabelen $X_i$, er lik null: 
$$\mathbb{E}[\epsilon_i \vert x_i]=0$$
Dette innebærer at det ikke må være noen systematisk sammenheng mellom restleddet og forklaringsvariabelen -- vilkåret forutsetter bl.a. at det ikke er noen korrelasjon mellom $\epsilon$ og $x$, slik at $\mathbb{E}[\epsilon_i x_i]=0$. Intuitivt krever eksogenitet at  oppdelingen av $y_i$ i ulike deler er "ren", dvs. at det vi putter i $\epsilon$-boksen virkelig er uavhengig av $x$. Hvis noe i $\epsilon$-boksen egentlig hører sammen med $x$, har vi et problem.
...
##### Eksempel 1: Fører sykehusopphold til dårligere helse? {.unnumbered}
...
I gjennomsnitt har en pasient innlagt på sykehus signifikant dårligere helsetilstand enn noen som ikke er innlagt på sykehus. Dersom vi skulle gjort en regresjon eller en hypotesetest, ville denne konkludert med at innlagte på sykehus har dårligere helse enn befolkningen forøvrig. Betyr det at sykehusbehandling *fører til* dårligere helsetilstand? Det høres lite sannsynlig ut. Det er mer nærliggende å tro at pasienter som er innlagt på sykehus, er innlagt fordi de *allerede* har dårligere helsetilstand enn befolkningen forøvrig. I dette eksempelet er altså $X_i$ og $\epsilon_i$ korrelert. Det er lettere å tro på omvendt kausalitet -- altså at det er den dårlige helsetilstanden som førte til sykehusoppholdet -- enn opprinnelige tolkningen om at sykehusoppholdet førte til dårligere helsetilstand! 
...
$$ \log w_i = \beta_0 + \beta_1 x_i + \epsilon_i$$ 
...
$$y_i=\beta_0+\beta_1x_i+\beta_2A_i+\epsilon_i$$
...
$$y_i=\beta_0+\beta_1x_i+\xi_i\qquad \text{der } \xi_i=\beta_2A_i+\epsilon_i$$
Fordi det er sammenheng mellom $A$ og $y$, inneholder det nye restleddet vårt $\xi$ nå også $A$. I avsnitt @sec-ekso så vi at vi trenger *eksogenitet* for å kunne trekke kausale slutninger, som krevde at det ikke var noen sammenheng mellom forklaringsvariabelen og restleddet ($\mathbb{E}[\epsilon_i\vert x_i]=0$). Dette ser vi tydelig at ikke kan være tilfelle når restleddet $\xi_i$ inneholder $A_i$. $A_i$ er jo korrelert med $x$, så da blir $\mathbb{E}[\xi_i\vert x_i]\neq0$, og eksogenitetsbetingelsen er brutt. Dette er en formell illustrasjon av *utelatt variabelskjevhet*, som vi diskuterte i [eksempel 2](#eks2): Når det er en utelatt variabel som er korrelert med både utfalls- og forklaringsvariabelen vår, fanger $\beta_1$ opp mer enn bare effekten av $x_i$ på $y_i$. 
...
$$ y_{it}=\beta_1x_{it}+\nu_t+\alpha_i+\eta_{kt}+\epsilon_{it}$$
...
Prøv deg på Oppgave 2 i den individuelle hjemmeeksamen H2020 som du finner i kapittel @sec-skoleeksamen. Det er spesielt oppgave e) og f) som er relatert til paneldata, men vi bemerker at oppgave f) var "nøtten" i det oppgavesettet.
...
$$\log(\text{Omsetning}_i) = \beta_0 + \beta_1 \log(\text{Reklamekroner}_i) + \epsilon_i$$
...
a) Forklar med egne ord hva eksogenitetsbetingelsen $\mathbb{E}[\epsilon_i \vert x_i]=0$ krever i denne sammenhengen. Hva må vi anta om restleddet?

b) Nevn minst to variabler som kan tenkes å ligge i restleddet $\epsilon_i$, og som sannsynligvis er korrelert med hvor mye butikken bruker på reklame. Tegn gjerne et DAG-diagram.
...
$$\log(\text{Omsetning}_i) = \beta_0 + \beta_1 \log(\text{Reklamekroner}_i) + \beta_2 \text{Sentrum}_i + \epsilon_i$$
...
$$\text{PM}_{10,it} = \beta_0 + \beta_1 \text{Bergen}_i + \beta_2 \text{Etter}_t + \beta_3 (\text{Bergen}_i \times \text{Etter}_t) + \epsilon_{it}$$
...
$$\log(\text{Produktivitet}_{it}) = \beta_0 + \beta_1 \text{NæringA}_i + \beta_2 \text{Etter2021}_t + \beta_3 (\text{NæringA}_i \times \text{Etter2021}_t) + \epsilon_{it}$$
...
## Relevante R-kommandoer {#sec-relevante-r-avansert-regresjon}
...
### Tilpasse en logistisk regresjonsmodell {.unnumbered}
...
### Trene en kNN-modell {.unnumbered}
